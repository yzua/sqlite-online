...
| `useSqlEditorConfig.ts`      | `useSqlEditorConfig`                        | Hook: returns CodeMirror extensions array. Suppresses SQL completions for `/ai` prompts (completion source returns null). |
...
2. `useSqlEditorConfig` suppresses SQL completions for `/ai` prompts (completion source returns null).
3. On Execute, `ExecuteTab` calls `generateSqlQuery()` from `useGeminiAI`.
4. `requestGeminiSql()` (in `@/lib/ai/gemini`) calls Gemini 1.5 Flash with full
   schema context.
