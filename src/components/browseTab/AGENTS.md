...
| `BrowseTabEditOverlay.tsx`  | `BrowseTabEditOverlay`  | Absolute-positioned edit overlay, conditionally renders `EditSection`.                                                    |
| `EditSection.tsx`           | `EditSection`           | Inline edit panel. Composes header, fields, and actions.                                                                  |
...
| `rowMeta.ts`                | `getRowMeta`            | Pure function: computes view-aware PK stripping and display data for a row.                                               |
...
BrowseTab
  ...
  +-- ResizablePanelGroup
        ...
        +-- schemaPanel: SchemaTree + BrowseTabEditOverlay
              +-- EditSection
                    +-- EditSectionHeader
...
- **Primary key handling**: `BrowseTableRow` delegates to `getRowMeta()` (from
  `rowMeta.ts`) to strip the PK column from display and extract the primary
  value for selection callbacks. View-aware: no PK stripping for views.
