...
| `EditOverlay.tsx`           | `EditOverlay`           | Mobile edit overlay. Wraps `EditSection` in an absolute-positioned container shown/hidden via `isEditing`.                                  |
| `EditSection.tsx`           | `EditSection`           | Inline edit panel. Composes header, fields, and actions. Rendered inside `EditOverlay` (both desktop schema panel and mobile overlay).      |
...
| `rowMeta.ts`                | `getRowMetas`           | Batch function: computes per-row PK value, display data, and stable row keys. View-aware (no PK extraction for views). Used by `DataTable`. |
...
BrowseTab
  ...
  +-- ResizablePanelGroup
        ...
        +-- schemaPanel (desktop): SchemaTreePanel + EditOverlay
        +-- mobile: EditOverlay
  EditOverlay
    +-- EditSection
          +-- EditSectionHeader
...
- **`usePanelSizing`** (hook): `dataPanelSize`, `schemaPanelSize`, setters. Wraps
  `usePanelStore`/`selectPanelSizes` internally.
...
- **`usePanelManager`** (context): `isEditing`, `isInserting`, `selectedRowObject`,
  `handleInsert`, `handleCloseEdit`, `handleRowClick`, `setEditValues`.
- **`useEditValues`** (hook): `editValues`.
...
- **Primary key handling**: `DataTable` uses `getRowMetas()` (from `rowMeta.ts`)
  to batch-compute per-row PK value, display data, and stable row keys.
  View-aware: no PK extraction for views.
