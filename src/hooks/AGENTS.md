...
| `usePanel.ts`       | `usePanelManager` (default), `useEditValues`, `usePanelActions`, `usePanelState` (named) | Consumers for split `PanelContext`. `usePanelManager` merges actions + state. `usePanelActions` returns callbacks only (stable refs). `usePanelState` returns selection state. `useEditValues` returns edit values context. |
